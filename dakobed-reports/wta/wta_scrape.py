...
def get_postgres_connection(db, schema = None):
    conn = psycopg2.connect("host=postgres dbname={} user=postgres password=postgres".format(db)) if not schema else \
        psycopg2.connect(host="postgres", port="5432", user="postgres",  password="postgres", database=db, options="-c search_path={}".format(schema))
    return conn
