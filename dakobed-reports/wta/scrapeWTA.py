...
def get_postgres_connection(db, schema = None):
    conn = psycopg2.connect("host={} dbname={} user=postgres password=postgres".format(host,db)) if not schema else \
        psycopg2.connect(host=host, port="5432", user="postgres",  password="postgres", database=db, options="-c search_path={}".format(schema))
    return conn
...
trp_report_table_insert = ("""INSERT INTO trip_reports (trip_name, trip_report, elevationGain, mileage, locations, trip_date) VALUES (%s, %s, %s, %s, %s, %s);""")

cur.execute(create_trip_report_table)
conn.commit()
...
#         report_list_url = 'https://www.wta.org/@@search_tripreport_listing?b_size=100&amp;b_start:int=%d&amp;_=1584045459199"' % int(100 * page_number)
#         try:
